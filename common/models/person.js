...
module.exports = function(Person) {
  ...
  Person.timetravel = function(cb) {
    Person.find({},(err, allPersons) => {
      allPersons.forEach((person) => {
        if(person.dates && person.dates.baptism) {
          console.log(person.dates.baptism);
          var d = new Date(person.dates.baptism);
          if(d.getTime() === d.getTime()) {
            d.setDate(d.getDate()+1);
            person.dates.baptism = d.toISOString();
            console.log("new", person.dates.baptism);
